<div class="post-item bg-white rounded-4 shadow-sm p-4 mb-4">
    ...
    @if ($posts)
    <p class="text-justify mb-4"><x-linkify-content :content="Str::limit($post->body, 75)" />
        @if (strlen($post->body) > 75)
        <a class="text-secondary text-decoration-none" href="/post/{{ $post->id }}">More Detail</a>
        @endif
    </p> 
    @else
    ...
    @endif       
        <div class="d-flex justify-content-end mt-4">
            <div class="btn-group align-end me-3" role="group" style="column-gap: 2px;">
                ...
                @endif
                @if ($commentDropdown)
                <button class="btn btn-outline-primary align-middle d-flex align-items-center" type="button" data-bs-toggle="collapse" data-bs-target="#commentSection-{{ $post->id }}" aria-expanded="false" aria-controls="commentSection-{{ $post->id }}">
                    <span class="material-icons align-middle">comment</span>
                </button>
                @endif
                @if ($post->user_id == Auth::id())
                <form action="{{ url('/post/delete') }}" method="post" style="display: inline-block;">
                    @csrf
                    @method('DELETE')
